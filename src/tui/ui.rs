...
fn render_logs(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let filter_line = Line::from(vec![
        Span::styled(
            format!(" [1] Endpoint: {} ", endpoint_filter_label(app)),
            Style::default().fg(Color::Cyan),
        ),
        Span::styled(
            format!(" [2] Event: {} ", event_filter_label(app)),
            Style::default().fg(Color::Cyan),
        ),
        Span::styled(
            format!(" [3] Status: {} ", status_filter_label(app)),
            Style::default().fg(Color::Cyan),
        ),
        Span::styled(
            format!(
...
fn endpoint_filter_label(app: &App) -> String {
    if app.filter_endpoint == 0 {
        "All".into()
    } else {
        app.endpoints
            .get(app.filter_endpoint - 1)
            .map(|e| e.name.clone())
            .unwrap_or_else(|| "All".into())
    }
}
fn event_filter_label(app: &App) -> String {
    if app.filter_event == 0 {
        "All".into()
    } else {
        app.event_types
            .get(app.filter_event - 1)
            .map(|e| e.name.clone())
            .unwrap_or_else(|| "All".into())
    }
}
...
fn render_help_bar(frame: &mut Frame, app: &App, area: Rect) {
    let help = match (&app.modal, &app.screen) {
        ...
        (ModalState::None, Screen::Endpoints) => {
            "Tab: switch | ↑↓/jk: nav | c: create | e: edit | d: delete | p: ping | q: quit"
        }
        (ModalState::None, Screen::DeliveryLogs) => {
            "Tab: switch | ↑↓/jk: nav | v: details | t: trigger | r: retry | l: listener | 1-3: filters | s: sort | x: clear | q: quit"
        }
        (ModalState::CreateWebhook | ModalState::EditWebhook(_), _) => {
            "Tab/↑↓: fields | ←→: Cancel/Submit | Space: toggle | Enter: activate | PgUp/PgDn: scroll | Esc: cancel"
        }
        ...
        (ModalState::ListenerConfig, _) => {
            "Tab/↑↓: fields | type URL | Space: toggle | Enter: activate | Esc: cancel"
        }
    };
    frame.render_widget(
...
fn render_toast(frame: &mut Frame, msg: &str) {
    let area = frame.area();
    let width = u16::try_from(msg.len())
        .unwrap_or(u16::MAX)
        .saturating_add(4)
        .min(area.width);
