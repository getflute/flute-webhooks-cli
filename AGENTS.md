...
|---|---|
| `webhooks endpoints list` | `{ "data": [GetWebhookEndpointDto, …] }` |
...
| `webhooks endpoints delete <id> --yes` | empty stdout, exit 0 |
| `webhooks endpoints ping <id>` | `PingResponseDto { success, status_code, duration_ms, error_message? }` |
| `webhooks event-types list` | `{ "data": [EventTypeDto, …] }` |
...
| `webhooks deliveries retry <id>` | `DeliveryLogSummaryDto` for the newly scheduled retry |
...
Field types are defined in [`src/api/models.rs`](src/api/models.rs). camelCase wire format.
...
- `"client"` → bad CLI args or unknown profile; treat as a programming error in the agent's invocation.
...
| `deliveries retry` | **no** | each call schedules an additional retry attempt. Check the latest log via `deliveries get` before retrying again. |
...
| "Create a webhook for transaction events" | `webhooks endpoints create --url <URL> --events transaction.card.captured,refund.completed --name "<name>"` |
| "Pause this endpoint" | `webhooks endpoints update <id> --status inactive` |
| "Delete this endpoint" | `webhooks endpoints delete <id> --yes` (the `--yes` flag is required — no interactive prompt) |
| "Is this endpoint reachable?" | `webhooks endpoints ping <id>` (returns `success: bool` + `status_code`) |
| "Show the last 50 deliveries" | `webhooks deliveries list --limit 50` |
| "Show the failures for a given endpoint" | `webhooks deliveries list --endpoint-id <id> --status failed` |
| "Inspect a specific delivery's payload" | `webhooks deliveries get <id>` |
| "Re-send a failed delivery" | `webhooks deliveries retry <id>` |
